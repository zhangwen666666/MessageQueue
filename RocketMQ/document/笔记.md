...
* 方案三：采用redis+自旋保证只有一个线程能拿到锁，执行后续的更新数据库操作，因此service中就可以不用做mysql行锁了，可以采用方案一的写法了。如果在给定的时间内没有拿到锁，就会结束了。此外，我们的redis锁可以设置一个过期防止，防止死锁的发生，防止某个线程在执行业务代码时卡死导致无法释放锁。
