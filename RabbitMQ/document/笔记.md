...
2. 在浏览器中访问http://111.119.211.126:15673/，登录用户名为admin，密码为admin。这个用户名和密码是创建docker容器时设置的。
...
* **镜像队列配置命令：**
  ...
  * `[Priority]`: 可选参数，表示策略的优先级。值越小，优先级越高。

* 比如想为虚拟主机 `zw` 配置所有名字开头为`policy_`的队列进行镜像，镜像数量为2，那么命令如下（在任意节点执行如下命令）：

  ```
  ./rabbitmqctl set_policy -p zw ha_policy "^policy_" '{"ha-mode":"exactly","ha-params":2,"ha-sync-mode":"automatic"}'
  ```
...
* 针对某个虚拟主机进行镜像
  ...
  ```
  rabbitmqctl set_policy -p zw ha-all "^" '{"ha-mode":"all","ha-sync-mode":"automatic"}'
  ```
...
![image-20250509205144059](F:\JavaCode\MessageQueue\RabbitMQ\document\image\image-20250509205144059.png)

此时web控制台上队列如下：

![image-20250509205315680](F:\JavaCode\MessageQueue\RabbitMQ\document\image\image-20250509205315680.png)

此时即使是`rabbit@rabbit1`停了，也不影响接收和发送消息。
